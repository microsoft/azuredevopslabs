...
1. Select the **Build & Release** tab.

   ![](images/001.png)

1. Select the **XAML** tab to view the project’s XAML build definitions. Here you can see the existing **Nightly Fabrikam (Dev)** XAML build definition that is currently used by the team to validate the development branch.
...
1. Select the **Roles** tab to view queue security. You can precisely define who can access these agents and how.
...
1. Select the **Agent Queues** tab and click **Download agent**.
...
1. Return to **Internet Explorer** and refresh the window to see the new **default** queue. Click **Manage pools**.
...
1. Select the **Build & Release** tab and click **New definition** to create a new build definition.
...
1. Return to the **Build** tab, select the **Visual Studio** template, and then click **Next**.

   ![](images/023.png)

1. Select **FabrikamFiber Team project** as the **Repository source** and click **Create** to create the build definition.
...
1. For **Multipliers**, enter **“BuildPlatform, BuildConfiguration”** in order to take into account these variables during the build.

   ![](images/027.png)

1. Enabling the **Parallel** option would allow you to distribute the jobs (one from each combination of values from Multipliers) to multiple agents in parallel (if available). However, since we only have one agent installed, we won’t be able to parallelize the build here.

1. Select the **Variables** tab. This list shows variables that will be available to all build steps (tasks). There are a number of predefined variables that can be used by tasks during the build, all listed [here](http://go.microsoft.com/fwlink/?LinkId=550988), as well as any additional variables that can be added in this view. By default, both **BuildConfiguration** and **BuildPlatform** are already defined here (we just referenced these in our configuration of Multipliers prior).
...
1. Modify the second default mapping manually (of type Cloak) to be **“$/FabrikamFiber/Dev/Drops**”.
...
1. Select the **General** tab. This is where you configure the default queue to use, build number format, build timeout, and other general settings. This is also where you can specify demands for specific agent capabilities. By default, we have demands already in place to ensure that MSBuild, Visual Studio, and VSTest capabilities are defined for the agent.

   ![](images/034.png)

1. Select the **Build** tab. The first build step restores any NuGet packages that might be out of date. There are a variety of options for configuring this, such as the ability to disable the local cache. Leave these options as defaults.

   ![](images/035.png)

1. The second build step defined by this template is for **Build solution**. By default, it is configured to build all solution files in all subfolders, use the platform specified by the BuildPlatform variable, and use the configuration specified by the BuildConfiguration variable. Ensure **Visual Studio “15” (preview)** is selected as the **Visual Studio Version** (this will build as Visual Studio 2017).

   ![](images/036.png)

1. The third build step is **Test Assemblies**. We will mostly use default options here to test assemblies with “*Fabrikam*test” in the name. Update the **Test Assembly** field to **“**\$(BuildConfiguration)\*Fabrikam*test*.dll;-:**\obj\**”**. Note that this is really just inserting “*Fabrikam” before “*test”.

   ![](images/037.png)

1. Expand the **Advanced Execution Options** section and note the additional options, such as the ability to specify a specific version of VSTest or paths to custom test adapters.
...
1. The fifth build step is **Copy Files** and places a copy of the build into a staging directory.
...
1. Select the Deploy tab and check out the available options that range from Azure Web Site deployment to PowerShell execution and file copy. For simplicity, let’s deploy to the local machine using the Windows Machine File Copy step, so click Add. Then Close the dialog.
...
1. In the **Select Path** window, expand and select **$/FabrikamFiber/Dev/FabrikamFiber.CallCenter/FabrikamFiber.Web**. Click **OK**.

   ![](images/055.png)

1. There is also an option here to trigger the build on a set schedule. For example, we could set this build up to run every evening if desired.
...
1. Make any change to the file and click **Save**.

   ![](images/059.png)

1. Select the **Build & Release** tab and open the **Fabrikam Development CI Build** by clicking it.
